...
### Monocular SLAM for people detection & path planning
- Developed a **custom PGO implementation** using GT-SAM to allow GPS to optimize streaming inference using feedforward visual SLAM models such as MapAnything, imbuing the map with metric scale and clean trajectory alignment.
...
{{< video "https://youtu.be/bJJkKbZUV9c"
    title='GPS PGO for starling drone' 
>}}
- The same system also serves to create accurate maps and pose estimations from monocular data, which in turn form the backbone of the person ReID system. Details currently confidential :). 
![A demo of the mapping based ReID system](images/projects/anyslam/starling_reid_demo.png){width=60% fig-align="right"}
